...
public class BoothWriter
{
	String flname;
	
	public BoothWriter(String flname)
	{
		this.flname = flname;
	}
	
	public void write(Vector<Booth> booths) throws IOException
	{
		Path path = Paths.get(flname);
		Writer writer = Files.newBufferedWriter(path, Charset.forName("US-ASCII"));
		
		writer.write("booths " + booths.size() + "\n");
		for(Booth booth : booths)
		{
			writer.write(booth.getPositionX() + " " + booth.getPositionY() + " " + booth.getRadius() + "\n");
		}
